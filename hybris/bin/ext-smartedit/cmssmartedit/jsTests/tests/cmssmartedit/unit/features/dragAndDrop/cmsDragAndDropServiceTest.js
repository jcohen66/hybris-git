...
describe('cmsDragAndDropService', function() {
    ...
    var SUPPORTED_BROWSERS_CONSTANT = {
        IE: 'IE',
        CHROME: 'Chrome',
        FIREFOX: 'Firefox',
        EDGE: 'Edge',
    ...
    beforeEach(function() {
        var harness = AngularUnitTestHelper
            ...
            .mock('browserService', 'getCurrentBrowser')
            .mockConstant('SUPPORTED_BROWSERS', SUPPORTED_BROWSERS_CONSTANT)
    ...
    describe('event handlers', function() {
        ...
        it('GIVEN the cursor is over the dragged element WHEN onDragOver is called THEN the component is highlighted', function() {
            // Arrange
            cmsDragAndDropService._dragInfo.componentId = initialValues.component.id;
        ...
        it('WHEN mouse leaves drag area THEN the slot is cleared', function() {
            ...
            expect(mocks.systemEventService.sendAsynchEvent.calls.count()).toBe(2);
            expect(mocks.systemEventService.sendAsynchEvent.calls.argsFor(0)[0]).toEqual('HIDE_SLOT_MENU');
            expect(mocks.systemEventService.sendAsynchEvent.calls.argsFor(1)[0]).toEqual('CMS_DRAG_LEAVE');
        });
    ...
    describe('onDrop', function() {
        ...
        it('WHEN a new component is dropped THEN the generic editor modal is displayed for it.', function() {
            ...
            // Assert
            expect(mocks.componentEditingFacade.addNewComponentToSlot).toHaveBeenCalledWith(slotInfo, undefined, dragInfo.componentType, expectedPosition);
        ...
        it('WHEN an existing component is dropped into a slot that already has an instance THEN an error message is displayed.', function() {
            ...
            // Assert
            expect(mocks.componentEditingFacade.addExistingComponentToSlot).toHaveBeenCalledWith(targetSlotId, dragInfo, expectedPosition);
            expect(cmsDragAndDropService.onStop).toHaveBeenCalled();
        });
        ...
        it('WHEN a new component is moved within the same slot before THEN page is updated accordingly.', function() {
            ...
            spyOn(cmsDragAndDropService, '_getComponentPositionInSlot').and.returnValue(currentPosition);
            cmsDragAndDropService._highlightedHint = highlightedHint;
            var expectedPosition = highlightedHint.position;
    ...
    it('GIVEN user is in Safari WHEN _getDragImageSrc is called THEN it returns the right image path', function() {
        ...
        mocks.browserService.getCurrentBrowser.and.returnValue(SUPPORTED_BROWSERS_CONSTANT.SAFARI);
    ...
    it('GIVEN user is not in Safari WHEN _getDragImageSrc is called THEN it returns empty image path', function() {
        ...
        // Act
        // - Chrome
        mocks.browserService.getCurrentBrowser.and.returnValue(SUPPORTED_BROWSERS_CONSTANT.CHROME);
        var chromeResult = cmsDragAndDropService._getDragImageSrc();

        // - Firefox
        mocks.browserService.getCurrentBrowser.and.returnValue(SUPPORTED_BROWSERS_CONSTANT.FIREFOX);
        var firefoxResult = cmsDragAndDropService._getDragImageSrc();

        // - Edge
        mocks.browserService.getCurrentBrowser.and.returnValue(SUPPORTED_BROWSERS_CONSTANT.EDGE);
        var edgeResult = cmsDragAndDropService._getDragImageSrc();

        // - IE
        mocks.browserService.getCurrentBrowser.and.returnValue(SUPPORTED_BROWSERS_CONSTANT.IE);
        var ieResult = cmsDragAndDropService._getDragImageSrc();

        // - Others
        mocks.browserService.getCurrentBrowser.and.returnValue(SUPPORTED_BROWSERS_CONSTANT.UNKNOWN);
        var otherResult = cmsDragAndDropService._getDragImageSrc();

        // Assert
        expect(mocks.browserService.getCurrentBrowser.calls.count()).toBe(5);
        expect(chromeResult).toBe(expectedResult, 'No drag image needed for Chrome');
        expect(firefoxResult).toBe(expectedResult, 'No drag image needed for Firefox');
        expect(edgeResult).toBe(expectedResult, 'No drag image needed for Edge');
        expect(ieResult).toBe(expectedResult, 'No drag image needed for IE');
        expect(otherResult).toBe(expectedResult, 'No drag image needed for unknown browsers');
    });
});
