...
module.exports = function() {
    ...
    function getFeatures(done) {
        return browser.get('jsTests/tests/cmssmarteditContainer/e2e/features/app.html').then(function() {
            if (done) {
                done();
            }
        });
    ...
    function getE2EConfigs() {
        var configDirs = getConfigDirs(PATHS.tests.cmssmarteditContainere2eTestsRoot);
        var configs = [];
        configDirs.forEach(function(configDir) {
            ...
            if (testFile) {
                ...
                try {
                    var path = configDir.substr(PATHS.tests.cmssmarteditContainere2eTestsRoot.length);
                    configs.push({
                        key: path + '/' + testFile,
    ...
    function getTestsListPageHtml(e2eConfigs) {
        var tplFileContent = fs.readFileSync(PATHS.tests.cmssmarteditContainere2eTestsRoot + '/features/list.html.tpl', 'utf8');
    ...
    function createTestsListPage() {
        ...
        var fileName = PATHS.tests.cmssmarteditContainere2eTestsRoot + '/features/list.html';
    ...
    return {
        loadE2eDependencies: function(browser, dependencyContainer) {
            ...
            browser.setStorefrontDelayConfigInSessionStorage = function(delayConfig) {
                browser.get('/jsTests/tests/cmssmarteditContainer/e2e/_shared/dummystorefront/fakeAngularEmptyPage.html');
             ...
             * /cmssmartedit/jsTests/tests/cmssmarteditContainer/e2e/features/util/commonMockedModule/configurationMocks.js
             */
            browser.bootstrap = function(specDir, done) {
                ...
                if (config) {
                    return browser.get('/jsTests/tests/cmssmarteditContainer/e2e/_shared/dummystorefront/fakeAngularEmptyPage.html').then(function() {
                        if (config.jsFiles) {
