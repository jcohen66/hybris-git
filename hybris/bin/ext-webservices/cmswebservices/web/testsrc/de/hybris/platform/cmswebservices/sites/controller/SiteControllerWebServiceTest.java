...
public class SiteControllerWebServiceTest extends ApiBaseIntegrationTest
{
	...
	public void setUp()
	{
		EXPECTED_APPAREL_SITE = allOf( //
				...
				hasProperty(THUMBNAIL_URL, equalTo(APPAREL.getThumbnailUri())),
				hasProperty(REDIRECT_URL, equalTo(APPAREL.getBaseUrl())));

		EXPECTED_ELECTRONICS_SITE = allOf( //
				hasProperty(NAME, equalTo(ELECTRONICS.getNames())), //
				hasProperty(UID, equalTo(ELECTRONICS.getUid())), //
				hasProperty(THUMBNAIL_URL, equalTo(ELECTRONICS.getThumbnailUri())),
				hasProperty(REDIRECT_URL, equalTo(ELECTRONICS.getBaseUrl())));
	}
